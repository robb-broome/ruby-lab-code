(define_conditions [
  ...
  (-1 "(SIBLING_CALL_P (insn)) && (Pmode == DImode)")
  ...
  (-1 "(TARGET_VZEROUPPER && SIBLING_CALL_P (insn)) && (Pmode == DImode)")
  (-1 "(DImode != DImode || TARGET_64BIT)
   && SSE_FLOAT_MODE_P (SFmode) && TARGET_SSE_MATH
   && !(TARGET_INTER_UNIT_CONVERSIONS || optimize_function_for_size_p (cfun))
   && reload_completed
  ...
  (-1 "(X87_FLOAT_MODE_P (GET_MODE (operands[1]))
   ...
   && !(SSE_FLOAT_MODE_P (GET_MODE (operands[1]))
	 && (TARGET_64BIT || DImode != DImode))
   && can_create_pseudo_p ()) && ( 1)")
  (-1 "((TARGET_VZEROUPPER && SIBLING_CALL_P (insn)) && (Pmode == DImode)) && ( reload_completed)")
  ...
  (-1 "(TARGET_SSE2 && reload_completed) && (Pmode == DImode)")
  (-1 "TARGET_USE_FANCY_MATH_387
   && flag_unsafe_math_optimizations")
  ...
  (-1 "(INTVAL (operands[0]) == GET_MODE_SIZE (Pmode)) && (Pmode == DImode)")
  (-1 "ix86_match_ccmode (insn, CCNOmode)
   && !(MEM_P (operands[0]) && MEM_P (operands[1]))")
  ...
  (-1 "reload_completed
   && (operands[2] = find_constant_src (insn))")
  (-1 "(INTVAL (operands[0]) == 2*GET_MODE_SIZE (Pmode)) && (Pmode == DImode)")
  (-1 "ix86_match_ccmode (insn, CCNOmode)
